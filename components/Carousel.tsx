...
const Carousel = ({ data }: CarouselProps) => (
	<div className='flex-none h-auto opacity-60 relative w-708 max-tablet:h-16 max-tablet:w-full max-tablet:px-3'>
		<div className='flex-main h-8 justify-center gap-2.5 w-full opacity-60 px-4 py-8 max-tablet:h-full max-tablet:gap-0 max-tablet:px-0'>
			<div className='overflow-visible flex-none opacity-100 h-[200px] relative w-[676px] max-tablet:w-full'>
				...
				>
					<ul className='animate-carousel flex shrink-0 items-center justify-center list-none flex-row gap-12'>
						{data.map((item) => (
							<li key={`${item.name} primary`}>
								<div className='text-white h-auto overflow-visible relative'>
									{item.name}
								</div>
							</li>
						))}
